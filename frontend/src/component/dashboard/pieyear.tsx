...
interface PieYearProps {
  gradeData: { value: number; name: string }[];
}

const PieYear: React.FC<PieYearProps> = ({ gradeData }) => {
  useEffect(() => {
    const chartDom = document.getElementById("pieyear");
    if (chartDom) {
        ...
        series: [
          {
            ...
            type: "pie",
            radius: "50%",
            data: transformedData.filter((data) => data.value > 0),
            emphasis: {
              itemStyle: {
                shadowBlur: 10,
                shadowOffsetX: 0,
